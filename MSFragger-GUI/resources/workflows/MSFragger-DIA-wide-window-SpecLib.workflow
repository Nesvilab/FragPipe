# Workflow: MSFragger-DIA-wide-window-SpecLib
...
msfragger.output_report_topN=6
...
workflow.description=<p style\="margin-top\: 0"> MSFragger-DIA based workflow for direct identification of peptides from wide-window DIA data (or combined narrow- and wide-window DIA data). Alternative workflow to using DIA-Umpire. Reporting 6 highest scoring hits for each MS/MS spectrum. FDR filtering to 1% at all levels (protein, peptide, PSM) using 2D, picked FDR filtering. Spectral library building with EasyPQP to generate a Spectronaut and DIA-NN compatible spectral library for subsequent quantification using those tools. RT alignment in EasyPQP using "Automatic selection of a run as RT Reference". </p>
