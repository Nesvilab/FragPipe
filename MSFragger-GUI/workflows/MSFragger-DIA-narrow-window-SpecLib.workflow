# Workflow: MSFragger-DIA-narrow-window-SpecLib
...
msfragger.misc.fragger.remove-precursor-range-hi=1
msfragger.misc.fragger.remove-precursor-range-lo=-1
...
workflow.description=<p style\="margin-top\: 0"> MSFragger-DIA mode for direct identification of peptides from DIA data. Recommended options for narrow-window DIA data such as GFP DIA data for building a spectral library. Reporting 3 highest scoring hits for each MS/MS spectrum. FDR filtering to 1% at all levels (protein, peptide, PSM) using 2D, picked FDR filtering. Spectral library building with EasyPQP to generate a Spectronaut and DIA-NN compatible spectral library for subsequent quantification using those tools. </p> \n<p style\="margin-top\: 0"> Default is RT alignment using commonly observed peptides (ciRT). Alternatively (recommended for non-human data), when building a library from narrow-window DIA runs, include one (or more) wide-window single-shot DIA runs, and choose "Automatic selection of a run as Reference' in EasyPQP. All runs will then be aligned to the reference wide-window DIA run. </p>
