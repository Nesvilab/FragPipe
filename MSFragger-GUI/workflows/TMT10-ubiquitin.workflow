...
morerescore.predict-rt=true
morerescore.predict-spectra=true
...
tmtintegrator.mod_tag=K(114.0429),K(343.2059)
...
workflow.description=<p style\="margin-top\: 0"> TMT 10-plex workflow for ubiquitin enriched data, with quantification from MS2. Site localization based on the MSFragger search engine assignment. TMT is specified as variable modification on K (fixed on n-term). Oxidation is specified as variable modifications. Ubiq is specified as two variable modifications\: +114 (Ubiq) and +343 (both Ubiq and TMT on the same K). Up to 3 missed cleavages and 4 variable modifications in total. PSM validation with Percolator, and protein inference with ProteinProphet. Data is filtered at 1% FDR at the PSM, ion, peptide, and protein levels. TMT-Integrator with virtual reference approach, median-centering normalization, data summarization at the gene/protein/peptide/site levels. If a reference/bridge sample is available, specify the corresponding channel/sample name tag in the annotation file(s) and in TMT-Integrator tab. </p>
