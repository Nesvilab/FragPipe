...
workflow.description=DIA-Umpire based workflow for direct DIA analysis and building spectral libraries. Supports RAW, mzML, and mzXML for Thermo data. Requires mzML files for AB Sciex and timsTOF PASEF data. Pseudo-MS/MS spectra are extracted with DIA-Umpire SE module, and searched using MSFragger. PSM validation with Percolator, protein inference with ProteinProphet via Philosopher. 1% FDR filtering at all levels (protein, peptide) using 2D, picked FDR strategy. EasyPQP for generating a spectral library compatible with Spectronaut and DIA-NN for subsequent quantification using those tools.
