...
//    private static final Pattern reSpecRankPercolator = Pattern.compile("([^\\\\/]+)_\\d+_(\\d+)$");
    ...
    private static final Pattern pattern0_spectrum_AKA_reSpecQuerySpectrum = Pattern.compile("(.+spectrum=\".+\\.)([0-9]+)\\.([0-9]+)(\\.[0-9]+\".+)");
    private static final Pattern pattern1_baseName = Pattern.compile("base_name=\"([^\"]+)\"");
    private static final Pattern pattern2_rawDataType = Pattern.compile("raw_data_type=\"([^\"]+)\"");
    private static final Pattern pattern3_rawData = Pattern.compile("raw_data=\"([^\"]+)\"");
    ...
    public static void percolatorToPepXML(final Path pin, final String basename, final Path percolatorTargetPsms,
        ...
        for (int rank = 1; rank <= (is_DIA ? max_rank : 1); ++rank) {
            ...
            // TODO: BEGIN-01 =========================
            // TODO: There's exactly the same piece of code in RewritePepxml class
            ...
            createInteractFile(se, minProb, is_DIA, pinSpectrumRankNttNmc, pinSpectrumRankPepScore, rank, output_rank, pepxml_rank, hasCalibratedFile);
        }
    }

    private static void createInteractFile(SearchEngine se, double minProb, boolean is_DIA, Map<String, NttNmc[]> pinSpectrumRankNttNmc, Map<String, PepScore[]> pinSpectrumRankPepScore, int rank, Path output_rank, Path pepxml_rank, boolean hasCalibratedFile) {
            ...
            while ((line = brpepxml.readLine()) != null) {
                if (line.trim().startsWith("<msms_run_summary")) {
                    if (hasCalibratedFile && line.contains("This pepXML was from calibrated spectra.")) {
                        Matcher matcher1 = pattern1_baseName.matcher(line);
                        if (matcher1.find()) {
                            line = matcher1.replaceFirst(Matcher.quoteReplacement("base_name=\"" + matcher1.group(1) + "_calibrated" + "\""));

                            Matcher matcher2 = pattern2_rawDataType.matcher(line);
                            if (matcher2.find()) {
                                line = matcher2.replaceFirst("raw_data_type=\"mzML\"");
                            }

                            Matcher matcher3 = pattern3_rawData.matcher(line);
                            if (matcher3.find()) {
                        ...
                        if (!extension.equalsIgnoreCase("mzml")) {
                            ...
                            if (matcher1.find()) {
                                ...
                                matcher2 = pattern2_rawDataType.matcher(line);
                                if (matcher2.find()) {
                                    line = matcher2.replaceFirst("raw_data_type=\"mzML\"");
                                }

                                matcher3 = pattern3_rawData.matcher(line);
                                if (matcher3.find()) {
                                    line = matcher3.replaceFirst("raw_data=\"mzML\"");
                                }
                            }
