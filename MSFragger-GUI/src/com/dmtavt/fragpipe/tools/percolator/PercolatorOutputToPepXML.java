...
public class PercolatorOutputToPepXML {
    ...
    public static void main(final String[] args) {
        ...
        if (args.length == 0)
            ...
            if (Files.exists(Paths.get(args[0].replace(".pin", "_edited.pin")))){
    ...
    private static StringBuilder handle_search_hit(final List<String> searchHit, final NttNmc nttNmc, final PepScore pepScore, final int oldRank, final int newRank) {
        ...
        {
            if (! Double.isNaN(nttNmc.spectralSimilarity)) {
                sb.append(
                        String.format(
                                "<search_score name=\"spectralsim\" value=\"%f\"/>\n",
                                nttNmc.spectralSimilarity
                        ));
            }
            if (! Double.isNaN(nttNmc.RTscore)) {
                sb.append(
                        String.format(
                                "<search_score name=\"rtscore\" value=\"%f\"/>\n",
                                nttNmc.RTscore
                        ));
            }
            sb.append(
    ...
    public static void percolatorToPepXML(final Path pin, final String basename, final Path percolatorTargetPsms, final Path percolatorDecoyPsms, final Path outBasename, final String DIA_DDA, final double minProb) {
        ...
        try {
            ...
            while ((line = brtsv.readLine()) != null) {
                ...
                double spectralSimilarity = Double.NaN;
                if (indexOf_spectralSimilarity != -1) {
                    spectralSimilarity = Double.parseDouble(split[indexOf_spectralSimilarity]);
                }
                double RTscore = Double.NaN;
                if (indexOf_RTscore != -1) {
                    RTscore = (Double.parseDouble(split[indexOf_RTscore]) - scoreMean) / scoreStd;
                }
    ...
    static class NttNmc {
        ...
        final int ntt;
        final int nmc;
        final double spectralSimilarity;
        final double RTscore;

        public NttNmc(int ntt, int nmc, double spectralSimilarity, double RTscore) {
            this.ntt = ntt;
            this.nmc = nmc;
