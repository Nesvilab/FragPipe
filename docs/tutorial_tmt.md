...
##### Tutorial contents
* [Download data](./tutorial_tmt#download-data)
* [Start FragPipe](./tutorial_tmt#start-fragpipe)
* [Set workflow and load files](./tutorial_tmt#set-workflow-and-load-files)
* [Fetch database](./tutorial_tmt#fetch-database)
* [MSFragger search settings](./tutorial_tmt#msfragger-search-settings)
* [Validation settings](./tutorial_tmt#validation-settings)
* [Quantification settings](./tutorial_tmt#quantification-settings)
* [Run the analysis](./tutorial_tmt#run-the-analysis)
* [Results](./tutorial_tmt#results)
...
### Start FragPipe
Find the `fragpipe` folder and open the `fragpipe/bin` subfolder. Double-click the `fragpipe.exe` file on Windows (or the `fragpipe` file on Linux) to launch the FragPipe program. See [this page](./tutorial_fragpipe#configure-fragpipe) for help configuring FragPipe. Once MSFragger, IonQuant, and Philosopher are configured, you are ready to start your analysis.
...
TMT-Integrator reports will be in a new folder, 'tmt-reports'. These reports contain normalized log2-transformed values, with separate files for both abundances (intensities) and ratios at each level (gene, protein, peptide, etc.). See the TMT/iTRAQ section on [this page](./tutorial_fragpipe_outputs) for more details.
