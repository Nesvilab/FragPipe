...
For partial processing (e.g. to save time upon re-analysis), steps can be skipped by unchecking the corresponding boxes. This tutorial walks through each tab in some detail, but once FragPipe is [configured](./tutorial_setup_fragpipe), analysis can be as simple as choosing spectral files, a database, and a workflow to run. Find a guide to output files [here](./tutorial_fragpipe_outputs).

Before you get started, make sure your LC-MS file format is compatible with the workflows you want to perform. For Thermo data (with or without FAIMS), we recommend [converting .raw files to mzML with peak picking](./tutorial_convert). DIA data acquired with overlapping/staggered windows must be [converted to mzML with demultiplexing](./tutorial_convert#convert-thermo-dia-raw-files-with-staggered-windows).
...
#### Tutorial contents
* [Configure FragPipe](./tutorial_fragpipe#configure-fragpipe)
* [Select workflow and add spectral files](./tutorial_fragpipe#select-workflow-and-add-spectral-files)
  * [Single-experiment report](./tutorial_fragpipe#single-experiment-report)
  * [Multi-experiment report](./tutorial_fragpipe#multi-experiment-report)
  * [Affinity purification data](./tutorial_fragpipe#affinity-purification-data)
  * [TMT/iTRAQ data](./tutorial_fragpipe#tmtitraq-data)
* [DIA-Umpire SE](./tutorial_fragpipe#run-dia-umpire-se)
* [Choose a sequence database](./tutorial_fragpipe#specify-a-protein-sequence-database)
* [Configure MSFragger search](./tutorial_fragpipe#configure-msfragger-search)
* [Validation](./tutorial_fragpipe#validation)
* [PTMs](./tutorial_fragpipe#ptms)
* [Label-free quantification](./tutorial_fragpipe#lfq-label-free-quantification)
* [Isobaric quantification](./tutorial_fragpipe#isobaric-labeling-based-quantification)
* [Spectral library generation](./tutorial_fragpipe#spectral-library-generation)
* [DIA quantification with DIA-NN](./tutorial_fragpipe#dia-quantification)
* [Run FragPipe](./tutorial_fragpipe#run-fragpipe)
<br/>
...
1. Choose the [workflow](./tutorial_fragpipe_workflows) you want to use from the dropdown menu and press 'Load'. Use the **Default** workflow for simple conventional (closed) searches. A number of common workflows (including [glyco](./tutorial_glyco) and [DIA](./tutorial_DIA)) are provided. We always recommend starting with one of the provided workflows and customizing it as needed. Customized workflows can be saved (with a unique name) for future use or sharing with other users (all workflows are stored in the FragPipe 'workflows' folder).
...
![](../images/fragpipe_tutorial-workflow.png)
...
#### TMT/iTRAQ data
For TMT/iTRAQ analysis, spectral files should be in mzML format (with peak picking, see the [conversion tutorial](./tutorial_convert)). Raw files are not currently supported.
...
### Run DIA-Umpire SE
[DIA-Umpire](https://diaumpire.nesvilab.org/)'s signal extraction module can be used for .raw and .mzML files without ion mobility. See this [tutorial](./tutorial_DIA) for more detail.
...
**Note about highly complex searches:** For non-specific searches or for searches with many variable modifications, you may need to use the database splitting option, for which Python must be installed (see [Configure FragPipe](./tutorial_fragpipe#configure-fragpipe) above).
...
![](../images/fragpipe_tutorial-labelquant.png)
...
![](../images/fragpipe_tutorial-labelquant-annotate.png)

Annotation files will be named 'annotation.txt' and saved in each folder. Only one 'annotation.txt' is allowed per folder, so we recommend separate folders for each plex (see [TMT/iTRAQ data](./tutorial_fragpipe#tmtitraq-data) above).
