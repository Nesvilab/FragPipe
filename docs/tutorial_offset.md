## Contents
* [Offset search for PTMs with FragPipe](./tutorial_offset#offset-search-for-ptms-with-fragpipe)
* [Types of Mass Offset Searches](./tutorial_offset#types-of-mass-offset-searches)
* [Options for Reporting Mass Offsets](./tutorial_offset#options-for-reporting-mass-offsets)
* [Example Mass Offset Search Tutorial (tubulin search)](./tutorial_offset#example-mass-offset-search-tutorial)
* [Histone Detailed Mass Offset (DMO) Search using the HiP-Frag method](./tutorial_DMO_histones_example)
...
<div align="center">
<img src="../images/closed-open-offset_search.png" width="600px" align="middle"/>
</div>
...
amino acid(s). Labile mode diagnostic and remainder ions can also be specified (see the [labile](./tutorial_labile) mode tutorial for more details). 
...
![](../images/offset_template_example.png)
<br/>
[Example template file](../images/offsets_example.tsv)
...
### Example contents
* [Add the data](./tutorial_offset#add-the-data)
* [Fetch a sequence database](./tutorial_offset#fetch-a-sequence-database)
* [Customize the search settings](./tutorial_offset#customize-the-search-settings)
* [Set the output location and run](./tutorial_offset#set-the-output-location-and-run)
* [Examine the results](./tutorial_offset#examine-the-results)


### Open FragPipe
When you launch FragPipe, check that MSFragger, IonQuant, and Philosopher are all configured. If you haven’t downloaded them yet, use their respective ‘Download / Update’ buttons. Please see the tutorials [here](./tutorial_fragpipe#configure-fragpipe) and [here](./tutorial_setup_fragpipe) for more help. Python is not needed for this exercise.
...
6) When we loaded the 'Mass-Offset-CommonPTMs' workflow, the [default list of offsets](./common_mass_offsets) includes more than we need for this analysis since we want to focus on parthenolide. In the 'Mass Offsets' field, replace the offsets list with just 0/248.14125 for the parthenolide adduct. The list of mass offsets can be `/` or space separated, and only the specified mass offsets will be allowed in the search (i.e. combinations of specified mass offsets will not be generated).
...
### Examine the results
In the output location, you will see subfolders containing individual results for DMSO and PTL, plus PTM-Shepherd output files that summarize the mass shifts from both experimental conditions together. A guide to all the output files can be found [here](./tutorial_fragpipe_outputs).
