...
For this tutorial, we will use one run ([`2014-03-14-NSN-38B-2`, download from PRIDE](https://www.ebi.ac.uk/pride/archive/projects/PXD000743)) from an FFPE-preserved sample of amyloid deposits in eye tissues, acquired on a SCIEX TripleTOF, originally in .wiff format that needs to be [converted to .mzML by ProteoWizard](./tutorial_convert). (Publication: Nielsen, Nadia Sukusu, et al. "Insight into the protein composition of immunoglobulin light chain deposits of eyelid, orbital and conjunctival amyloidosis." Journal of proteomics & bioinformatics (2014).)


##### Tutorial contents
* [Open FragPipe](./tutorial_lfq#open-fragpipe)
* [Add the data](./tutorial_open#add-the-data)
* [Load the Open workflow](./tutorial_open#load-the-open-workflow)
* [Fetch a sequence database](./tutorial_open#fetch-a-sequence-database)
* [Inspect the search settings](./tutorial_open#inspect-the-search-settings)
* [Set the output location and run](./tutorial_open#set-the-output-location-and-run)
* [Examine the results](./tutorial_open#examine-the-results)
